...
export default function SuperAdmin() {
  ...
  const filteredCompanies = companies.filter(company => {
    ...
    const matchesStatus = statusFilter === 'all' || company.is_active === (statusFilter === 'active');
  ...
  const handleToggleCompanyStatus = async (tenantId: string, isActive: boolean) => {
    try {
      await api.updateTenantStatus(tenantId, isActive);
      showSuccessToast(`Company ${isActive ? 'activated' : 'deactivated'} successfully`);
      fetchCompaniesAndStats(); // Refresh data
    } catch (error) {
  ...
  const handleUpdateAdmin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (adminEditForm.first_name) updateData.first_name = adminEditForm.first_name;
      if (adminEditForm.last_name) updateData.last_name = adminEditForm.last_name;
      if (adminEditForm.new_password) updateData.password = adminEditForm.new_password;

      const response = await fetch(`/api/super-admin/companies/${editingCompany.id}/admin`, {
        method: 'PUT',
        headers: {
              ...
              <CardContent>
                ...
                {loading ? (
                  <div className="flex items-center justify-center py-8">
                    ...
                    <span className="ml-2 text-gray-700">Loading companies...</span>
                  </div>
                      ...
                      {filteredCompanies.map((company) => (
                        <React.Fragment key={company.id}>
                          <TableRow>
                            ...
                            <TableCell className="text-sm text-gray-900">{company.domain}</TableCell>
                            <TableCell>
                              {company.admin ? (
                                <div>
                                  <div className="font-medium text-gray-900">{company.admin.first_name} {company.admin.last_name}</div>
                                  <div className="text-sm text-gray-600">{company.admin.email}</div>
                                </div>
                              ) : (
                            ...
                            <TableCell>
                              <div className="flex gap-2">
                                <Button variant="ghost" size="sm" onClick={() => handleEditCompany(company)} title="Edit Company">
                                  <Edit className="w-4 h-4" />
                                </Button>
                                <Button variant="ghost" size="sm" onClick={() => handleEditAdmin(company)} title="Edit Admin" >
                                  <UserPlus className="w-4 h-4" />
                                </Button>
                                <Button variant="ghost" size="sm" onClick={() => handleDeleteCompany(company)} title="Delete Company" className="text-red-600 hover:text-red-700">
                                  <Trash2 className="w-4 h-4" />
                                </Button>
                                <Button variant="ghost" size="sm" onClick={() => handleToggleCompanyStatus(company.id, !company.is_active)} className={company.is_active ? 'text-yellow-600 hover:text-yellow-700' : 'text-green-600 hover:text-green-700'} title={company.is_active ? 'Deactivate Company' : 'Activate Company'}>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <Input
                                ...
                                onChange={(e) => setCompanyForm({ ...companyForm, admin_first_name: e.target.value })}
                                required
                              />
                            </div>
                            <div>
                              ...
                              <Input
                                ...
                                onChange={(e) => setCompanyForm({ ...companyForm, admin_last_name: e.target.value })}
                                required
                              />
                            </div>
                          </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => setAdminEditForm({ ...adminEditForm, first_name: e.target.value })}
                        ...
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) => setAdminEditForm({ ...adminEditForm, last_name: e.target.value })}
                            placeholder="Leave empty"
                            className="text-xs h-8 focus:ring-2 focus:ring-blue-500/20 bg-white/70"
                          />
                        </div>
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <Input
                            ...
                            onChange={(e) => setAdminEditForm({ ...adminEditForm, new_password: e.target.value })}
              ...
              <Card className="bg-white/95 backdrop-blur-md shadow-lg">
                <CardContent className="p-4">
                  <div className="flex items-center mb-3">
                    ...
                    <div>
                      <h3 className="text-sm font-semibold text-gray-900">Delete Company</h3>
                  ...
                  <div className="mb-4 p-3 bg-gray-50 rounded-md border border-gray-200">
                    ...
                    {deletingCompany.admin_email && (
                      <p className="text-xs text-gray-600 mt-2">
                        <span className="font-medium">Admin:</span> {deletingCompany.admin_email}
                      </p>
                    )}
