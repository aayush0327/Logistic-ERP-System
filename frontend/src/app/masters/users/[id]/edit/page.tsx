...
export default function EditUserPage() {
  ...
  const onSubmit = async (data: UserUpdateFormData) => {
    ...
    try {
      ...
      if (data.branch_id !== user?.branch_id) updateData.branch_id = data.branch_id || undefined;
      ...
      toast.error(error?.data?.detail || error?.message || 'Failed to update user');
    } finally {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="text-sm text-gray-600">
                <p>Profile Type: <span className="font-medium capitalize">{user.profile_type}</span></p>
                <p>Superuser: <span className="font-medium">{user.is_superuser ? 'Yes' : 'No'}</span></p>
          ...
          <div className="flex justify-end space-x-3">
            <Button
              type="button"
              variant="outline"
              onClick={() => router.push(`/masters/users/${userId}`)}
