...
import { useGetProductQuery, useGetProductStockHistoryQuery } from '@/services/api/companyApi';
...
export default function ProductDetailsPage() {
      ...
      <AppLayout>
        <div className="p-6">
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <h3 className="text-red-800 font-medium">Error loading product details</h3>
            <p className="text-red-600 text-sm mt-1">The product may not exist or you don't have permission to view it</p>
          </div>
        </div>
      </AppLayout>
    );
  ...
  const getStockStatus = () => {
    if (!product.max_stock_level) return { status: 'unknown', label: 'Not Set' };
  ...
  const stockPercentage = product.max_stock_level ? (product.current_stock / product.max_stock_level) * 100 : 0;
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <Button onClick={() => router.push(`/masters/products/${productId}/edit`)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">${product.unit_price.toFixed(2)}</p>
              </div>
              {product.special_price && (
                <div>
                  <label className="text-sm font-medium text-gray-500">Special Price</label>
                  <p className="text-lg font-semibold text-green-600">${product.special_price.toFixed(2)}</p>
                </div>
              )}
              <div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <div>
                <label className="text-sm font-medium text-gray-500">Min Level</label>
                <p className="text-lg text-gray-900">{product.min_stock_level || 0}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-500">Max Level</label>
                <p className="text-lg text-gray-900">{product.max_stock_level || 0}</p>
              </div>
              <div className="pt-2">
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${
                      ...
                      stockStatus.status === 'normal' ? 'bg-green-500' : 'bg-blue-500'
                    }`}
                    style={{ width: `${Math.min(stockPercentage, 100)}%` }}
                  ></div>
                </div>
                <p className="text-xs text-gray-500 mt-1">{stockPercentage.toFixed(1)}% of max stock</p>
              </div>
            </CardContent>
          </Card>
              ...
              {product.length && (
                <div>
                  ...
                  <p className="text-gray-900">
                    {product.length} × {product.width || 0} × {product.height || 0} cm
                  </p>
                </div>
              )}
              {product.handling_requirements && product.handling_requirements.length > 0 && (
                <div>
                  <label className="text-sm font-medium text-gray-500">Handling</label>
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between items-center p-3 bg-blue-50 rounded-lg">
                      ...
                      <span className="text-lg font-bold text-blue-600">+130</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600">Days of Stock Left</span>
                      <span className="text-lg font-semibold">
                        {product.max_stock_level ? Math.floor(product.current_stock / (product.max_stock_level * 0.1)) : 0} days
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600">Reorder Point</span>
                      <span className="text-lg font-semibold">{product.min_stock_level || 0}</span>
                    </div>
                    <div className="flex justify-between items-center">
                ...
                <Table>
                  ...
                  <TableBody>
                    <TableRow>
                      ...
                      <TableCell>${(50 * product.unit_price).toFixed(2)}</TableCell>
                      <TableCell>
                    ...
                    <TableRow>
                      ...
                      <TableCell>${product.special_price?.toFixed(2) || product.unit_price.toFixed(2)}</TableCell>
                      <TableCell>${(100 * (product.special_price || product.unit_price)).toFixed(2)}</TableCell>
                      <TableCell>
                    ...
                    <TableRow>
                      ...
                      <TableCell>${product.unit_price.toFixed(2)}</TableCell>
                      <TableCell>${(75 * product.unit_price).toFixed(2)}</TableCell>
                      <TableCell>
